...
无法判断属于哪类生成意图时【不要返回 plan】,按普通创作顾问对话回答(只给 reply)。`,
  ...
  { name: '节拍板拆解人设:独立键 beat.system,缺省逐字节等于内联原字面、覆盖只换人设句', fn() {
    ...
    assertEq(new Set(Prompts.list().map(x => x.def)).size, Prompts.list().length, '注册表各条 def 互不相同(同一句话不该有两份拷贝)');
  ...
  { name: '节拍板拆解人设(源级):js/beatboard.js 零内联;全仓内联人设持有者名单', fn() {
    ...
    assertEq(holders.length, 15, '持有者文件数');
    assertEq(holders.reduce((n, x) => n + Number(x.split(':')[1]), 0), 27, '全仓内联人设处数');
  } },
