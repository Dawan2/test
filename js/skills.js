  ...
  const REG = [
    ...
    },
    {
      ...
      note: 'G-12 在本条的落点已接上:发布留痕从"两端各一份实现、都在领域命令注册表之外"改成一条已注册命令 '
        ...
        + '两处 produce 喂进去的候选也逐档同序(命令入参 → 分集 sbConfig → 缺省),'
        + 'headless 那一侧不再漏掉分集参数配置面板上调过的那一档。'
        + '仍欠(G-03):两端闭环的调度粒度仍不同构——浏览器 autoSmartReview 把重试嵌在逐镜循环里'
        + '(一镜连着重抽到达标为止,进度面板逐镜播报、逐镜计费),CLI produce 是整集低分子集重抽一轮再复审(集外循环);'
        + '逐镜可重抽次数由同一个上限定死故两端得数相同,不同的是排法与回执的说法(一边报「每镜最多 N 次」、'
        + '一边报「第 x/N 轮」);要合成一份得先定浏览器侧进度面板语义与逐镜计费节奏能否改成分轮,属产品口径,尚未定',
    },
    {
