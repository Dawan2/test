...
- 缺口:复用既有 `G-01…G-15` 全部 15 项,另新登记 **8 项 `S-01…S-08`**(第 7 节;`S-08` 由整合时按统一编号规则从判定标准文档的 `G-16` 提议改记而来)。新缺口全部是"既有能力缺可判定出口"类,没有一项要求新增页面或新增创作功能。
...
| S-08 | 发布后无回写上游的回路:成片数据无法归因到主线哪一步,结论也不回流到项目策划侧 | 成片 | `Issues`/`Plans`/`bus` 有承接面,但没有"发布后 → 上游"的写回通道 | 待登记 | 待排期 |
