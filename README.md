...
**`exec <command>`(统一领域命令,与前端 `Commands.execute` 同名同结构)**:`hujing exec episode.preflight --pid X --epid Y`、`project.splitEpisodes`(整部剧本拆集,`--overwrite` 授权覆盖现有分集、`--local` 强制段落均分零计费)、`episode.generateVideos`(支持 `--confirm-all` 授权全量)、`shot.generateVideo`(`--sid` 支持镜头 id 或序号)、`episode.smartReview`、`episode.compose`、`episode.produce`(一键成片编排)。stdout 恒为 `{ ok, status, result?, error?, cost?, next? }`:`cost` 是钱包余额前后差值(含子调用扣费与退费回补),`next` 执行后按 Domain 重推下一步;exit 映射 `ok→0 | blocked→2(no-credits→6 / not-found→4) | failed→5`,blocked 前置拦截(缺剧本/未确认镜/无素材)零调用零计费。**二十一轮:LLM 创作类命令(智能分镜/本集理解/智能审片)已下沉服务端工作流端点 `/api/wf/*`(见 API 表)——CLI/外部 Agent 全链路可跑,计费动作服务端定死,提示词/规整与浏览器 js/wf-core.js 同源。二十四轮补齐主线前段:`project-script` + `exec project.splitEpisodes` 让 headless 能从"一份整部剧本"起跑到分镜表,不必先在浏览器手动拆集。**
