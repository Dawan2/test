...
const releaseTests = [
  ...
  { name: '交付包 · 下载失败仍照旧兜底,不因本轮新增回报改回假成功', fn: async () => {
    /* 守 W154 交接里那半:抓分镜这类"包内缺件"记 summary,而"包落不下来"那一路
     * (Blob/URL 不可用时的 ZipUtil.download 兜底)判据一字不动,不许借这轮顺手抹平。 */
    const rel = fs.readFileSync(path.join(ROOT, 'js/release.js'), 'utf8');
    const seg = rel.slice(rel.indexOf('async function downloadReleaseZip'), rel.indexOf('function execFix'));
    assert(/catch \(_\) \{ ZipUtil\.download\(name, \[\{ name: 'project_meta\.json', data: '空下载兜底:请重新打包' \}\]\); \}/.test(seg),
      '下载失败那一路仍走空下载兜底(不许改成静默成功)');
...
无法判断属于哪类生成意图时【不要返回 plan】,按普通创作顾问对话回答(只给 reply)。`,
  ...
  { name: '三套件用例数只增不减:unit/integration/cli.smoke 各自不低于下限(真删测且把 README 一并改小即红)', fn() {
    ...
    [['单元测试', 513, Object.values(SUITES).reduce((n, t) => n + t.length, 0), /单元测试[((](\d+) 项断言/g],
