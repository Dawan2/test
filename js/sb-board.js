...
(function () {
  ...
  function bindScriptBoard(host, p, ep, main, inModal) {
    ...
    // 拆解为文字分镜:LLM 把每个节拍拆成 1-3 条文字分镜,页内展示(不跳页);离线按句粗拆
    const draftBtn = host.querySelector('[data-x=bd-draft]');
    if (draftBtn) draftBtn.onclick = async () => {
      const localSplit = () => bd.scenes.forEach(sc => sc.beats.forEach(b => {
...
${brief.slice(0, 6000)}` }],
    ...
    if (aiBtn) aiBtn.onclick = async () => {
      if (!API.isReady()) return U.toast('AI 拆解需要真实 LLM(请登录后端)', 'error', 3000);
...
${(ep.content || '').slice(0, 8000)}` }],
        ...
        U.toast('AI 拆解失败:' + e.message, 'error', 3500);
        aiBtn.disabled = false; aiBtn.textContent = '✨ AI 拆解场次与节拍';
      }
    };
    const toBtn = host.querySelector('[data-x=bd-toshots]');
    if (toBtn) toBtn.onclick = () => {
      const beatsTotal = bd.scenes.reduce((n, sc) => n + sc.beats.length, 0);
      if (!beatsTotal) return U.toast('请先添加场次与节拍', 'error');
      const doIt = () => {
        const charNames = p.subjects.filter(s => s.kind === 'character').map(s => s.name);
        const shots = [];
        bd.scenes.forEach(sc => sc.beats.forEach(b => {
      ...
      if (ep.shots.length) U.confirm(`当前已有 ${ep.shots.length} 个分镜,转换将覆盖,确定继续吗?`, doIt, '覆盖转换');
      else doIt();
    };
  }
