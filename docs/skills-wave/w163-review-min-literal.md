...
| 6 | 把发布门 `DEFAULT_MIN_SCORE` 收编成 `Domain.REVIEW_MIN` | 红 1 —— 反向那句(收编前先改 §2.1 的记账) |
| 7 | 把提示词三档改成派生 | 红 1 —— 反向那句 |
| 8 | **对照项**:`D.REVIEW_MIN` 7 → 8,只同轮改那一句产品口径断言 | **全绿**,且闭环、重抽入口、状态条、回流镜数一并按 8 走 —— 这正是本槽要的实况(基线上同一手也全绿,但闭环停在 7) |

第 8 条是对照项不是凑数:它与 §1.2 那次变异**是同一手**,基线上全绿是缺陷、
现在全绿是修好了。区分这两种"全绿"的不是测试结果本身,而是第 1 条——
基线上第 1 条根本不存在(那时字面 7 就是实现),现在它在,
所以"整体挪一格全绿"才读得成"全线跟着挪了"而不是"没人在看"。
...
1. **要挪达标线**:改 `js/domain.js` 的 `D.REVIEW_MIN` 与 `tests/unit.js` 里
   `assertEq(sb.Domain.REVIEW_MIN, …)` 那一句,两处,别处不必动;全套跑一遍应全绿。
   若挪到 8 之后源级那条报「js/review.js 不得再按达标数 8 自判一遍达标」,
   那是撞上了直方图的 `>= 8` 色阶——那一处确实与新达标线同数了,该当场重判它算哪一档,
   不要把断言放宽。
