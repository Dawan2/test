...
浏览器 SPA(hash 路由,30+ 个 js 模块)
   ...
   │                 校验型扩展点 CHECKS 已落地十一条:剧本面 SK-07/08/09(开篇钩子锚定 / 打脸
   │                 四步完备性 / 台词单句长度),主体面 SK-11 主体参考纪律(按真实生成请求的参考
...
- **方法论校验命中(只读)**:审片报告在三维评分与关键问题之外另列一区「方法论校验命中」——按镜现取 `Skills.check`(js/skills.js 已落地的剧本面/主体面校验项,纯本地零 LLM 零积分),只收 `hits` 带本镜 `shotId` 的命中(台词单句超上限、引用主体解析不到、主体无真实参考图、本镜零主体引用),集级结论仍归就绪检查与问题中心不重复报;LLM 与离线本地评审同附,报告导出 TXT 同列。**只提醒不拦**:命中落在报告的独立字段 `checks`,不并入 `issues`——不参与三维/四维评分、达标线 7.0、修订重抽与批量优化判定,也不进发布门计数;旧报告无该字段按空处理,不回补冒充当时结论。
