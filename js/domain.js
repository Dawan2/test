...
(function (root, factory) {
   ...
   * 末句先给去重那条出口(浏览器主体库页面的按钮与 CLI 那条命令同读一份规则,故只点一个名字),
   * 再给回主体库人工改这条路,连它的已知代价一并说清:主体库的删除按 id 匹配
  ...
  D.dupSubjectRowsNote = function (picked, subs) {
    ...
    return '点名的 ' + picked.length + ' 位主体在主体库里同 id 存着多位(' + dup.map(x => x.id + ' ' + x.n + ' 位').join('、')
      ...
      + '要一个 id 只跑一位,先用 subjects-dedupe 收拾存量重复(主体库页面「🧹 主体 id 去重」按钮或 CLI 同名命令,'
   ...
   * 引用面因此一个字不用动:两侧按 id 的引用是 find 首位/首行语义,而首位留的就是原 id,
   * 去重前后落到的是同一位/同一行;改 id 的只有那些按 id 引用都指不到的后续位。
   * 一处例外是分镜侧的整集审片报告:lastReview.perShot 按行出条目、行对位走 reviewRows 的序数
   * (第几条同 id = 第几行同 id),同 id 只剩一行之后落在首行之外的那几条会退回首行;
   * 本扫描不改它一个字,回位靠去重后重跑整集审片(浏览器那条入口另把会塌几条现算现报在预览里)。
  ...
  /* 同 id 那几处里留原 id 的那一处排在第几号(只从 dupIdMarks 那张表上取,不再扫第二遍表)。
   * 位次那张表按序号取、答的是「这一处是第几处」,答不出「第 1 处在哪一格」;而卡片那一端读到
   * 「第 2/3 位」之后的下一句正是"第 1 位是哪一张"——引用解析落到的就是那一处,跳过去看得见的
   * 是名字与设定,而撞的那个 id 在卡片上看不见。收在这里而不是各页面自己数一遍:与位次同一套序,
   * 两份数法一漂,跳转就落到另一处上(而用户以为看的是引用会落到的那一位)。
   * 表里没有这个 id 的第 1 处(或入参脏)时回 -1,调用方那一枚小标就不挂跳转。 */
  D.dupIdKeepOrder = function (marks, id) {
    const t = marks && typeof marks === 'object' ? marks : {};
    const at = Object.keys(t).find(k => t[k] && t[k].id === id && t[k].nth === 1);
    return at === undefined ? -1 : Number(at);
  };
   ...
   * `ok` 的口径不改——它记的是引擎调用成功次数,计费也按这个数走(每次调用都真花了钱,
  ...
  D.landedNote = function (okCnt, landed, unit) {
    ...
    return '这一趟 ' + o + ' 次调用成功,产物只落到 ' + l + ' ' + u + '(landed):同 id 的多轮写到了同一' + u + '上,'
      ...
      + 'ok 记的是引擎调用成功次数(计费同口径,这几次都真花了钱),到手几' + u + '看 landed。';
  };
  /* ================= 镜头配音(TTS 渲染清单) =================
