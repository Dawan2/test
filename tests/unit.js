...
const commandsTests = [
  ...
  /* 提取的输入是「整本原文,没有整本才退回各集正文」,与门槛派生的「剧本这一步走过没有」不是同一问:
   * extractDone 的老项目上两问结论相反,而这一步要的是前者。判据收在 Domain.extractSourceText 一处,
   * 三端命令入口与计划层同读——命令层退回内联 p.script,下面那个桩当场看得出来。 */
  { name: 'extractSubjects:提取输入现取 Domain.extractSourceText(extractDone 老项目无原文无分集仍 blocked no-script)', fn: async () => {
    const sb = loadCommands();
    const { p } = cmdCtx(sb);
    p.episodes = [];
    p.subjects = [];
    p.extractDone = true; // 老项目:流程条上剧本步画 ✓,整本原文却没入库,分集也还没建
    assert(!sb.Domain.gateBlockers(p).some(g => g.code === 'no-script'), '前提:门槛派生认这类项目的剧本步已过');
    let r = await sb.Commands.execute('project.extractSubjects', { pid: 'p1' });
    assertEq(r.status, 'blocked'); assertEq(r.error.code, 'no-script', 'extractDone 这个进度位不是原文,提取读不到东西');
    assertEq(sb.__called.filter(c => /[eE]xtractSubjects/.test(c)).length, 0, '拿不到文本不应进入提取');
    // 输入取的是派生:项目对象一字不改,把派生打桩成读得到原文,同一次调用就能真跑
    const real = sb.Domain.extractSourceText;
    sb.Domain.extractSourceText = () => '女主在宴会厅被当众羞辱。';
    r = await sb.Commands.execute('project.extractSubjects', { pid: 'p1' });
    sb.Domain.extractSourceText = real;
    assertEq(r.ok, true, '实际:' + JSON.stringify(r.error || {}));
    assert(sb.__called.includes('extractSubjects'), '应把派生回的那份文本交给提取(离线启发式)');
    assertEq(sb.Domain.extractSourceText(p), '', '桩撤掉后这个项目仍然读不到原文(用例没有偷偷写 p.script)');
    // 各集正文那条回退没被收掉:整本仍缺,但分集正文在,提取照旧真跑
    p.episodes = [{ id: 'ep_x', title: '第1集', content: '女主赴宴,当众受辱。', shots: [] }];
    assertEq(sb.Domain.projectScript(p), '', '前提:整本原文仍然不在库');
    r = await sb.Commands.execute('project.extractSubjects', { pid: 'p1' });
    assertEq(r.ok, true, '各集正文是提取的回退输入,不该连坐被拦:' + JSON.stringify(r.error || {}));
  } },
...
const plansTests = [
  ...
  { name: 'fromWorkflow:剧本这一步与流程条同口径(提取过主体的老项目不再一步都推不出来)', fn() {
    ...
    /* 两步在这类项目上都是导航步:提取可读的原文一处也没有(整本没入库、分集也还没建),
     * 拆集的整本原文同样不在库、切分没有输入;两个两态各由下面一条专钉 */
    assertEq(pl.steps.map(s => s.cmd || 'goto:' + s.key).join(','), 'goto:extract,goto:split',
  ...
  /* 上一条把提取过主体的老项目从"整份计划推不出来"救了回来,可它给的提取步未必跑得动:
   * 门槛派生答的是「剧本这一步走过没有」(extractDone 也算),提取真正读入的是整本原文、
   * 没有整本才退回各集正文。两者在"提取过一轮却没留下原文"的项目上结论相反——
   * 有分集正文时提取确实能真跑(那半照旧出命令步),一个分集都没有时它注定 blocked/no-script,
   * 点下去只换来一句"请先上传剧本"。这里钉三态分流,且判据取的是命令层同读的那一份派生。 */
  { name: 'fromWorkflow:提取主体步按可读原文分两态(整本/各集正文都没有时出补原文导航步,不推注定 blocked 的命令步)', fn() {
    const sb = loadPlans();
    // ① 整本原文在库:照旧是可执行的命令步
    const withScript = { id: 'p1', script: '整本剧本', subjects: [], episodes: [] };
    let steps = sb.Plans.fromWorkflow(withScript).steps;
    assertEq(steps[0].cmd, 'project.extractSubjects', '实际:' + steps.map(s => s.label).join(' / '));
    assert(!steps[0].goto, '命令步不带 goto');
    // ② 老项目:提取过主体(门槛派生认剧本步已过)、整本原文没入库、一个分集也没有 → 提取读不到任何原文
    const legacy = { id: 'p1', extractDone: true, subjects: [], episodes: [] };
    assert(!sb.Domain.gateBlockers(legacy).some(g => g.code === 'no-script'), '前提:门槛派生认这类老项目的剧本步已过');
    assertEq(sb.Domain.extractSourceText(legacy), '', '前提:同一个项目上提取没有可读的输入');
    steps = sb.Plans.fromWorkflow(legacy).steps;
    assertEq(steps[0].key, 'extract', '这一步照出不藏,仍占提取那一步的位置');
    assertEq(steps[0].cmd, undefined, '注定 blocked/no-script 的命令不该挂上去');
    assertEq(steps[0].goto, '#/project/p1', '应导航到能补原文的项目页');
    assert(steps[0].label.includes('剧本原文'), '步骤文案应点名缺的是剧本原文;实际:' + steps[0].label);
    // ③ 同样没有整本原文,但分集正文在:提取按回退读得到文本,这一半照旧出命令步(不许连坐藏成导航步)
    const byEps = { id: 'p1', extractDone: true, subjects: [], episodes: [{ id: 'ep1', title: '第1集', content: '女主赴宴', shots: [] }] };
    assertEq(sb.Plans.fromWorkflow(byEps).steps[0].cmd, 'project.extractSubjects', '各集正文就是提取的回退输入,这一步真跑得动');
    // 分流判据现取派生:项目对象一字不改,只把派生打桩成"读得到原文",②那一步立刻变回命令步
    const real = sb.Domain.extractSourceText;
    sb.Domain.extractSourceText = () => '桩:整本原文';
    assertEq(sb.Plans.fromWorkflow(legacy).steps[0].cmd, 'project.extractSubjects', '内联判 p.script 拿不到这个桩');
    sb.Domain.extractSourceText = real;
    assertEq(sb.Domain.extractSourceText(legacy), '', '桩撤掉后这个项目仍然读不到原文(用例没有偷偷写 p.script)');
  } },
...
const releaseTests = [
  ...
  /* ---- 底部那处空 catch 的覆盖面(W181,W189 按转发被摘的新实况重算) ----
   * W149 判过它"无害"并留了三条理由,但没落判据;此后 episodes/issues-ui/workbench 各自开了通配订阅,
   * 三条理由里"下游是空的"那条已经只对一半(命名订阅仍是 0,通配那几家收得到)。W181 把它的覆盖面按
   * 源级 + 行为两面钉死,结论是「填掉它是拿十项门去换一个成品页走不到的形态」:
   ...
   * W183 走的不是"填掉"这条路,是把它兜的那句注册连同转发一起摘了:兜的对象没了,catch 跟着走,
   * 而 W181 那条结论落得更硬 —— 半截 Bus 现在压根伤不到这支模块,不必再靠一个 catch 兜。
   * 下面五条按 W181 立的五个面原样保留,各自照它们自己写下的"同轮重算"口径换到新实况上:
   * 覆盖面(本条)、门禁不变(下一条)、可达形态(第三条)、headless 无同一张网(第四条,一字未动)、
   * 留痕代价(第五条)。 */
  { name: 'Bus 通配订阅 · 那处 catch 连同它兜的那句注册一起摘了(源级:零空 catch、装载期零 Bus.on;行为:半截 Bus 也导得出整支)', fn() {
    const rel = fs.readFileSync(path.join(ROOT, 'js/release.js'), 'utf8');
    const empties = rel.match(/catch\s*\([^)]*\)\s*\{\s*\}/g) || [];
    assertEq(empties.length, 0, 'js/release.js 不该再有空 catch:W181 量过它兜的只有底部那一句订阅注册,' +
      '注册被摘掉之后它没有兜的对象了;再空一处就是另开了个吞口,实际 ' + empties.length);
    // 装载期一次订阅都不注册 —— 于是"注册失手"这个形态从根上不存在,不必再靠 catch 兜
    const code = blankNonCode(rel, true);
    assertEq((code.match(/Bus\.on\(/g) || []).length, 0,
      'release.js 装载期不得再注册任何订阅(要加回来就得同轮把兜它的那处 catch 一并加回来,并重新论证下游)');
    assert(/window\.Bus/.test(code), 'Bus 仍是这支模块的依赖(打版本落定发 release.stamped),不是整条断了');
    // 行为面按 W181 原样:Bus 在场时全函数;Bus 只有半截时整支照常导出、十项门一门不少
    const real = loadRelease('real');
    assertEq(typeof real.Bus.on('x', () => {}), 'function', 'Bus.on 是全函数,返回退订句柄');
    const degraded = loadRelease('no-on');
    assertEq(typeof degraded.Release, 'object', '半截 Bus 不该带走整支模块:把裸注册加回去而不兜住,这里当场是 undefined');
  ...
  { name: 'Bus 通配订阅 · 总线降不降级,G1–G10 结论逐字不变(它改不了任何一门,也不吞回调异常)', fn() {
    /* 「失败被吃掉、用户看到成功」在这条链上不成立:门禁每次现取,与总线是不是半截无关。
     * 同一夹具在"总线正常"与"总线只有半截(W181 那个 no-on 形态)"两种形态下把十门连同
     * overall/fails/warns/blockers/score 全量比一遍——只要哪天有人把降级路径接进门禁结论,这里当场红。 */
    ...
    assertEq(b, a, '总线降级与否,十门与四个汇总数须逐字相同');
    assert(/g4-stale:pass/.test(a) && /g9-subjects:fail/.test(a), '夹具须真摊到 pass 与 fail 两侧(全 pass 的夹具比了等于没比):' + a);
    /* 回调面的异常:归总线隔离,不反弹给管线。W181 拿 release.dirty 的订阅者数这一格,转发摘掉后
     * 改数真事件的通配订阅者——量的是同一件事(隔离在 js/bus.js 的 emit 里,不在 release.js 那侧)。 */
    const sb = loadRelease('real');
    sb.Bus.on('*', () => { throw new Error('角标重算崩了'); });
    let after = 0;
    sb.Bus.on('*', () => { after++; });
  ...
  { name: 'Bus 通配订阅 · 半截 Bus 那个形态在成品页不存在(Bus 只此一处整体赋值,且比 release 先装)', fn() {
    /* W181 量的那个形态是「window.Bus 在场而 Bus.on 不是函数」。bus.js 全文最后一句一次性把
     * 四个方法整体挂上去,全树再无第二处给 Bus 赋值,index.html 里它又排在 release 之前——
     * 于是这个形态只存在于测试桩里。真要出现,收通配的那几家连 catch 都没有、当场 TypeError,
     * 页面早塌在前面。W183 摘掉 release.js 那句注册后它连塌都不塌了,但这条守的是形态本身仍不可达。 */
    const jsFiles = fs.readdirSync(path.join(ROOT, 'js')).filter(f => /\.js$/.test(f));
    ...
    // 仍在收通配的这三家:没有 catch,缺 on 时整支模块当场没了 —— release.js 现在不在这张名单上
    const wild = jsFiles.filter(f => /Bus\.on\(\s*'\*'/.test(fs.readFileSync(path.join(ROOT, 'js', f), 'utf8')));
    assertEq(wild.sort().join(','), 'episodes.js,issues-ui.js,workbench.js',
      '通配订阅现在是这三家(release.js 已摘);名单变了就同轮重掂量半截 Bus 会波及谁');
    ['issues-ui.js', 'workbench.js'].forEach(f => {
      ...
      assert(err && /Bus\.on is not a function/.test(err),
        f + ' 在同一形态下当场抛(它没有任何 catch)——说明这个形态一旦为真,页面塌的不止一支,实际:' + err);
    });
  } },
  ...
  { name: 'Bus 通配订阅 · 转发件的代价重算:摘掉后一条主线事件占 1 格,release.js 只剩 release.stamped 一个发出点', fn() {
    /* W181 把转发件的代价登记成「一条主线事件占 2 格、50 格里真事件只剩一半」,并写明
     * 「转发若被摘掉或有人加命名订阅者,同轮在这里重算,别让它悄悄漂」。W183 把转发摘了,这里就是那一轮重算:
     * 一条主线事件占 1 格,连发 40 条就是 40 格真事件。留在本条的还有一格上一条判据没有的东西——
     * release.js 剩下的那个发出点是 release.stamped,它是打版本落定的**真事件**,占一格是留痕该干的活;
     * 转发件与真事件这条界线目前没有别处钉,再往这个文件里加发出点须先说清订阅者是谁。 */
    const sb = loadRelease('real');
    sb.Bus.emit('shots.batchDone', { brief: '批量出片完成' });
    assertEq(sb.Bus.recent(50).map(h => h.name).join(','), 'shots.batchDone',
      '一条主线事件占 1 格(转发回来时这里是 release.dirty,shots.batchDone 两格)');
    const sb2 = loadRelease('real');
    for (let i = 0; i < 40; i++) sb2.Bus.emit('shots.batchDone', { brief: 'b' + i });
    const hist = sb2.Bus.recent(50);
    assertEq(hist.length, 40, '连发 40 条主线事件就占 40 格,没有第二份占位(转发件在时这里当场满 50)');
    assertEq(hist.filter(h => h.name === 'shots.batchDone').length, 40, '40 格全是真事件(转发件在时真事件只剩 25 条)');
    const code = blankNonCode(fs.readFileSync(path.join(ROOT, 'js/release.js'), 'utf8'), true);
    assertEq([...code.matchAll(/Bus\.emit\('([^']+)'/g)].map(m => m[1]).join(','), 'release.stamped',
      'js/release.js 只该在打版本落定时发那一条真事件');
  } },
 ...
 * keepText 为真时只抹注释、字面量原样留着:护栏主题的锚点大多写在断言的取值与消息字面里,
 * 那一路要的是"注释不算数"而不是"只剩骨架"(注释里提一句锚点不该算成这条护栏还在)。 */
function blankNonCode(src, keepText) {
  const out = src.split('');
  const wipe = (a, b) => { for (let i = a; i < b && i < out.length; i++) if (out[i] !== '\n') out[i] = ' '; };
  const wipeText = (a, b) => { if (!keepText) wipe(a, b); };
...
/* ---- 护栏主题清单:给「这道护栏还在不在」一个不随用例名走的落点 ----
 * 「用例名成集双向比对」是本目录的零吃测手段,而它把两件事报成同一形态(都只是"只在基线里有的 N 条"):
 * 一条护栏按新实况翻面重写并改了名(该放行——W150 那次改写覆盖面反而变大),与一条护栏被真删掉(该拦)。
...
const GUARD_TOPICS = [
  { id: 'sk25-g03-ledger', anchors: ['review.reviseLoop', '仍欠(G-03)'],
    why: 'SK-25 的 G-03 两向对账:已落地那半与仍欠那半不许互串(W136 立、W150 翻面重写并改名)' },
  { id: 'sk26-g11-ledger', anchors: ['review.memoryFeedback', '仍欠(G-11)', 'core.memoryDual'],
    why: 'SK-26 的 G-11 记账与实况同步:回流面清 pending、自进化面仍欠,SK-04 那处余量跟着改' },
  { id: 'gaps-key-set', anchors: ['Skills.gaps()', 'w1-selected-skills.md'],
    why: 'gaps() 键集逐字点名并与两份缺口图谱双向对账(摘键与改名都先红在那条)' },
  { id: 'gate-blockers-single', anchors: ['Domain.gateBlockers', 'subject-no-image'],
    why: '前置门槛码只在 Domain.gateBlockers 一处,消费侧按码投影不另写(码名分裂过一次)' },
  { id: 'plans-gate-read', anchors: ['Domain.gateBlockers(p)', 'plans.js'], hosts: 2,
    why: '计划层不许回潮成第三份门槛判据:按码取材且码须是派生真会回的码(两处承载:计划层那条钉「不自己判剧本/主体/分集」,W153 那条逐码钉住扇出到计划层有没有投影)' },
  { id: 'revise-subset-single', anchors: ['WfCore.reviseSubset', 'lowShots'],
...
/* 一条主题在给定用例集上的判词。单独成函数是为了能拿造出来的用例集直接喂它——「抽掉某条承载用例会不会
 * 点名主题编号」这件事本身要有判据,而真删一条用例再跑全套不是单测做得到的事。
 * 落点数与登记不符的几种失败处置方向相反,故判词分开,报错句不许落到同一句上(一律以主题编号打头):
...
const contractTests = [
  ...
  { name: '提取主体的输入文本单源:Domain.extractSourceText 一处,三端命令入口与计划层同读', fn() {
    /* 门槛派生答的是「剧本这一步走过没有」(extractDone 也算走过),提取要的是「有没有可读的原文」——
     * 提取过一轮却没留下原文、连分集都还没建的老项目上两问结论相反,而这是有意的,不能靠改门槛去抹平。
     * 要收的是第二问的判据:它此前在三端命令入口各抄了一遍,计划层则据第一问推步,推出来的那一步注定 blocked。
     * 收进 Domain.extractSourceText 之后四个消费方同读一份;谁退回内联 p.script,入口条件就再次分叉。 */
    const Domain = require('../js/domain.js');
    assertEq(typeof Domain.extractSourceText, 'function', 'Domain 应导出提取主体的输入取数口');
    assertEq(Domain.extractSourceText({ script: '  整本  ' }), '整本', '整本原文优先,取到的是去空白后的正文');
    assertEq(Domain.extractSourceText({ episodes: [{ content: '第一集正文' }, { content: '' }, { content: '第二集正文' }] }),
      '第一集正文\n第二集正文', '没有整本时退回各集正文拼接(空集不占行)');
    assertEq(Domain.extractSourceText({ extractDone: true, subjects: [{ name: '甲' }], scriptMeta: { synopsis: '梗概' } }), '',
      '提取结论(主体库/卖点梗概)都不是原文:它们是产物,提不出逐字证据');
    const legacy = { id: 'p', extractDone: true, subjects: [], episodes: [] };
    assert(!Domain.gateBlockers(legacy).some(g => g.code === 'no-script'), '同一个项目:门槛派生认剧本这一步已过');
    assertEq(Domain.extractSourceText(legacy), '', '同一个项目:提取读不到原文——两问结论不同是有意的');
    /* 三端命令入口 + 计划层逐处点名:段落各按自己的入口标记切,切不到就先红在长度上 */
    const seg = (rel, from, to) => {
      const src = fs.readFileSync(path.join(ROOT, rel), 'utf8');
      const i = src.indexOf(from);
      assert(i > 0, rel + ' 里找不到提取入口标记 ' + from);
      const j = src.indexOf(to, i + from.length);
      assert(j > i, rel + ' 里找不到提取入口的收尾标记 ' + to);
      return src.slice(i, j);
    };
    [['js/commands.js', "reg('project.extractSubjects'", "reg('project.release'"],
      ['cli.js', "EXEC['project.extractSubjects']", "EXEC['"],
      ['server.js', "pathname === '/api/wf/extract-subjects'", "pathname === '/api/wf/"],
    ].forEach(([rel, from, to]) => {
      const body = seg(rel, from, to);
      assertEq((body.match(/p\.script/g) || []).length, 0, rel + ' 的提取入口不得再自己读 p.script');
      assert(/Domain\.extractSourceText\(p\)/.test(body), rel + ' 的提取输入应现取 Domain.extractSourceText');
    });
    assert(fs.readFileSync(path.join(ROOT, 'js', 'plans.js'), 'utf8').includes('Domain.extractSourceText(p)'),
      '计划层的提取步应按同一份派生分流(出命令步还是出补原文导航步)');
  } },
...
无法判断属于哪类生成意图时【不要返回 plan】,按普通创作顾问对话回答(只给 reply)。`,
  ...
  { name: '三套件用例数只增不减:unit/integration/cli.smoke 各自不低于下限(真删测且把 README 一并改小即红)', fn() {
    ...
    [['单元测试', 609, Object.values(SUITES).reduce((n, t) => n + t.length, 0), /单元测试[((](\d+) 项断言/g],
      ['集成测试', 146, reportLines('tests/integration.js'), /服务器级集成测试[^)]*扩至 (\d+) 项断言/g],
  ...
  { name: '护栏主题不许失联:清单里每条主题都还找得到承载它的用例(同主题改名翻面照旧绿,真删掉当场点名)', fn() {
    /* 上一条把"名集口径不被重名吃掉"钉住了,可名集比对本身还有一面分不出来:一条护栏被翻面重写并改了名
     * (该放行)与被真删掉(该拦),在比对结果里都只是"只在基线里有的 1 条"。W151 是本线第一次遇到前者,
     * 靠人工逐条核对新旧断言才判定它没被削弱,那一步至今零机器判据。
  ...
  { name: '护栏主题清单自身不许被架空:锚点须落在断言里、条数只增不减、取数口失效先红在这里', fn() {
     ...
     * 这里立三层:清单条数只增不减(销号得先改这个字面)、每条主题的锚点写法不许退化成恒真、
    ...
    GUARD_TOPICS.forEach(t => {
      if (!t.why) weak.push(t.id + ':缺 why(这道护栏守的是什么得写下来,否则销号时没人判得出该不该销)');
      if ((t.anchors || []).length < 2) weak.push(t.id + ':锚点少于 2 个(单锚点太容易被一句改写绕开,也太容易撞上别的用例)');
    ...
    GUARD_TOPICS.forEach(t => guardHostsOf(t.anchors).forEach(h => {
      const n = (h.code.match(/(?:^|[^\w$.])assert(?:Eq|NoThrow)?\s*\(/g) || []).length;
      if (n < 2) hollow.push(t.id + ' → ' + h.label + '(断言 ' + n + ' 处)');
    }));
    assertEq(hollow.join(' / '), '', '护栏主题的承载用例断言过少:锚点留着而判据被掏空,与直接删掉是同一件事');
    // 取数口自检:抹注释那一口失效(抹过头或没抹到)时,上一条会整条退化,故在这里先红
    assert(!blankNonCode('/* 锚点自检样例 */ const a = 1;', true).includes('锚点自检样例'),
      '注释没被抹掉:锚点写在注释里也会被算成落点,失联那条就此形同虚设');
    assert(blankNonCode("const s = '锚点自检样例';", true).includes('锚点自检样例'),
