...
浏览器 SPA(hash 路由,30+ 个 js 模块)
   ...
   │                 校验型扩展点 CHECKS 已落地三条:主体面 SK-12 分镜引用主体完备性(单镜引用
   │                 能否落到主体、有无可喂模型的真实参考图)与 SK-13 跨镜头主体一致性(同一主体
   │                 跨镜锁到的参考图/引用名一不一致),成片面 SK-28 字幕时间轴与阅读速度
   │                 (按合成时间轴段判烧录截断/读速/停留/空字幕轨),判定现取 Domain,经就绪检查
   │                 回执 result.checks 与问题中心消费,只报不拦、零 LLM 零计费
   ...
   │                 参考不一致/成片字幕读不顺 全项目聚合(Domain 与 skill 校验项单源推导),
...
- **🩺 问题中心**(js/issues.js,第三阶段):`Issues.collect(p)` 把全项目待处理问题聚合为分级清单(高危:失败镜/缺剧本;中:未分镜/分镜过期/素材过期/低分审片/成片过期/主体缺图;低:待确认/跨镜主体参考不一致/剧本方法论提醒/分集方法论提醒/成片字幕读不顺),逐项由 `Domain.episodeState` 推导——与流程条/下一步/跑批同一口径不自造状态;跨镜一致性一项由 `Skills.check('subjects',…)`(SK-13 校验项)推导,剧本方法论提醒(开篇钩子锚定/打脸四步/台词单句长度)由 `Skills.check('script',…)`(SK-07/08/09 校验项)推导,分集方法论提醒由 `Skills.check('eps',…)`(SK-14/SK-15 校验项)推导——六阶段结构覆盖判定输入是整张分集表故按项目挂一条、付费卡点位置按集挂,字幕可读性一项由 `Skills.check('film',…)`(SK-28 校验项)推导;剧本面与分集面都排在未拆镜等早退分支之前(剧本刚写完还没拆镜时正该看得见),本层只做展示文案,低危不影响发布门 G2;命令类问题一键处置(失败镜 → `episode.generateVideos` 带 `shotIds` 子集只重生成失败镜、未分镜 → `episode.generateStoryboard`、成片过期 → `episode.compose`,均 ui 模式保留决策闸),导航类跳对应页面。项目页 tab 行「🩺 问题」按钮(角标=未解决数,`Bus '*'` 订阅实时刷新),弹窗开着时管线事件驱动即时重算。
