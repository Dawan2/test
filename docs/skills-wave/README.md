...
- 资产不缺,缺索引:知识在 `js/knowledge.js`(18 条目)、提示词在 `js/prompts.js`(9 条)、人设在 `js/experts-data.js`(16 专家)、编排在 `js/cmd-registry.js`,层与层之间的按主线步骤索引由 `js/skills.js`(30 条内部能力)承担。**KB 条目的登记面已收成契约**:`KB.SECTIONS` 与索引宿主 SK-01 的 `kb` 双向逐条对齐、宿主内不重复登记、不得出现第二个全库宿主,新增条目漏登即红并点名缺哪个键(见 [w30-kb-skill-cover.md](./w30-kb-skill-cover.md))。
