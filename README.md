...
- **专家方法论进创作工作流(二十四轮)**:雇佣的专家不再只对对话面板生效——本集理解/智能分镜/智能审片/提取主体各条工作流的提示词按「生效专家」注入一段方法论(通道与导演设定同款,persona 截 200 字,未雇佣时提示词与原来逐字节一致)。生效顺序 **板块雇佣专家 > 全局雇佣专家**,与助手身份解析同序;板块对应关系 `WfCore.WF_BOARD`(理解→导演、分镜→分镜、审片→成片、提取主体→主体)单一来源。装配口双端各一处(浏览器 `personaNoteFor`、服务端 `wfPersonaNote`)且都调 `WfCore.personaFor`,同一雇佣状态下浏览器与 `/api/wf/*`(CLI `exec episode.understanding|generateStoryboard|smartReview|project.extractSubjects` 与 MCP 同链路)提示词逐字节一致。
