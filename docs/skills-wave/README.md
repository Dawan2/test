...
- 记账诚实位:`SK-03`/`SK-04`/`SK-23` 的 `infra` 面 `pending` 已按实况清空(G-01/G-02/G-03 的三处出口都在,由断言钉住实况),三条 `note` 改写为「已落地的出口 + **仍欠的覆盖余量**」并逐处点名——审片侧三步不带人设与记忆召回、记忆补种与旧板块名迁移仍只在浏览器发生、审片步尚无校验面。缺口标记按关联索引口径保留;沿用四波的那句「改 `pending` 会动 `Skills.gaps()` 投影」**实测不成立**(`gaps()` 只投影 `gaps` 字段、不看 `pending`),清账的行为面为零,见 [w36-infra-pending-align.md](./w36-infra-pending-align.md)。
