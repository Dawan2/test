...
const contractTests = [
  ...
  { name: '导演设定生成人设(源级):js/gsettings.js 零内联,全仓内联人设持有者名单精确到文件:处数', fn() {
    ...
    assertEq(inlinePersonaHolders.join(', '),
      'js/agent-ops.js:2, js/experts.js:2, js/plans.js:1, '
...
无法判断属于哪类生成意图时【不要返回 plan】,按普通创作顾问对话回答(只给 reply)。`,
  ...
  { name: '节拍板拆解人设(源级):js/beatboard.js 零内联;全仓内联人设持有者名单', fn() {
    ...
    assertEq(holders.join(' '),
      'js/agent-ops.js:2 js/experts.js:2 js/plans.js:1 js/proj-planner.js:2 '
  ...
  { name: '漫剧气泡对白人设:经 Prompts.get(comic.bubbleSystem) 取值,缺省逐字节等于收编前的内联字面', fn() {
    ...
    assertEq(census.join(' '), [
      'js/agent-ops.js:2', 'js/api.js:2', 'js/experts-data.js:16', 'js/experts.js:2',
    ...
    assertEq(Prompts.list().filter(x => x.def.startsWith('你是')).length, 29,
      '名单里 js/prompts.js 那 29 处就是注册表 def 本身(注册表条数变了这张名单也要跟着改)');
  } },
