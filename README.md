...
- **🩺 问题中心**(js/issues.js,第三阶段):`Issues.collect(p)` 把全项目待处理问题聚合为分级清单(高危:失败镜/缺剧本;中:未分镜/分镜过期/素材过期/低分审片/成片过期/主体缺图;低:待确认/跨镜主体参考不一致),逐项由 `Domain.episodeState` 推导——与流程条/下一步/跑批同一口径不自造状态;跨镜一致性一项由 `Skills.check('subjects',…)`(SK-13 校验项)推导,本层只做展示文案,低危不影响发布门 G2;命令类问题一键处置(失败镜 → `episode.generateVideos` 带 `shotIds` 子集只重生成失败镜、未分镜 → `episode.generateStoryboard`、成片过期 → `episode.compose`,均 ui 模式保留决策闸),导航类跳对应页面。项目页 tab 行「🩺 问题」按钮(角标=未解决数,`Bus '*'` 订阅实时刷新),弹窗开着时管线事件驱动即时重算。
