...
浏览器 SPA(hash 路由,30+ 个 js 模块)
   ...
   │                 校验型扩展点 CHECKS 已落地主体面两条:SK-12 分镜引用主体完备性(单镜引用
   │                 能否落到主体、有无可喂模型的真实参考图)与 SK-13 跨镜头主体一致性(同一主体
   │                 跨镜锁到的参考图/引用名一不一致),判定现取 Domain,经就绪检查回执
   │                 result.checks 与问题中心消费,只报不拦、零 LLM 零计费
   ...
   │                 参考不一致 全项目聚合(Domain 与 skill 校验项单源推导),
...
- **主线 skill 索引(js/skills.js)**:把知识条目(KB)、可覆盖提示词(Prompts)、提示词模板三件套、领域命令(CmdRegistry)、专家(ExpertsData)五类既有资产按主线七步(剧本/主体/分集/分镜/生成/审片/成片)加贯通层(`stage='*'`)登记为一张索引表——**短名单 30 条内部能力 `SK-01…SK-30`**,覆盖 KB 全部 17 条方法论、6 条注册表提示词、8 条领域命令与 16 位专家(新增单源键必须进索引,契约测试逐类断言零遗漏)。条目字段 `{id,sk,name,stage,covers,wave,kinds,pending,kb,kbBlocks,prompts,settings,cmds,experts,checks,steps,gaps}` **只存引用键**,正文仍只在 knowledge.js / prompts.js 各一份;`kinds` 记这条能力由哪几种机制面构成(inject 注入 / check 校验 / orchestrate 编排 / infra 基础设施),`pending` 如实记其中哪几面在主线上还没有出口(必须同时用 `gaps` 写明缺口编号),**未落地的面一律不挂假出口**:pending 含 check 的条目不得登记校验项、含 orchestrate 的不得登记步骤、含 inject 的不进拼块。`Skills.block(stage,{ids,sep})` 按索引现取 KB 拼注入块(与现有硬编码注入点逐字节一致),`Skills.playbook(id)`/`playbooks()` 给编排型 skill 的命令步骤(只引用已注册命令名),`Skills.check(stage,obj,ctx)` 跑校验型扩展点 `CHECKS`(纯本地、零 LLM、零计费,`obj` 收领域对象包 `{p,ep,s}`,逐项回 `{id,skill,pass,level,hits}`;**剧本面已落地三条**(判定输入是剧本正文本身——有分集上下文取该集正文、无分集取项目剧本原文,一律在去空白正文上判,结论全为 `warn`):SK-07 开篇钩子锚定 `script.openingHookAnchor`——开篇窗口(120 字)内有没有冲突锚点(直接进台词,或命中钩子六型的字面冲突信号),背景铺陈过长判 `late-hook` 并给出全文首个信号的位置,全文找不到锚点判 `no-hook-anchor`;SK-08 打脸四步完备性 `script.faceslapStepOrder`——羞辱/隐忍/反击/释放四步各按字面信号取首现位置,缺步判 `missing-step`、某步写在前一已命中步之前判 `step-out-of-order` 并给出基准步,命中步数不足两步即视为本集本就不是打脸段落而不产出结论(不拿"没写打脸"当缺陷报);SK-09 台词单句长度 `script.dialogueLineLength`——剧本正文的引号台词与分镜 `s.dialogue` 两处载体按句切分逐句判长,阈值现取 KB「对话铁律」条目正文(不在校验层写第二份数字),`hits` 定位到镜号或正文位置并带句首摘要。三条只判文本层看得见的编排缺失,钩型选得好不好、四步篇幅配比、潜台词与说明文式台词等语义面仍归 LLM 审片(G-10);**主体面已落地两条**:SK-12 分镜引用主体完备性 `subjects.shotRefIntegrity`——逐镜看 `characters/scene/props` 的引用名能否落到主体库、落到的主体有没有可喂模型的真实参考图,解析不到判 `fail`、缺参考图与零主体引用判 `warn`,`hits` 定位到镜号与名字;SK-13 跨镜头主体一致性 `subjects.crossShotConsistency`——同一主体被多镜引用时只看跨镜差异,该镜喂到的参考图与基准镜不是同一张(`ref-image-drift`)、别的镜锁得住这镜锁不住(`ref-lock-gap`,分该形态无真实图 `no-image` 与被参考图组上限挤出 `over-cap` 两因)、同形态跨镜引用名不统一(`alias-drift`,改名后旧名残留/别名混用)一律判 `warn`,基准取跨镜多数派,单镜绝对缺陷仍归 SK-12 不重复报;主体按名查找、取图与每镜真实参考图组口径现取 `Domain`(`findSubject`/`subjectRefImage`/`shotRefImages`),不在 skill 层写第二份;**分集面已落地两条**(判定输入是分集表本身,集序取 `p.episodes` 数组序):SK-14 六阶段结构覆盖 `eps.stageCoverage`——六段段名与集号区间现取 KB「六阶段结构」条目正文解析(校验层不写第二份区间数字),按条目刻度比例摊到当前集数,某段区间内没有一集有判得动的正文判 `stage-uncovered`、有集却不足按集数摊到的字数份额三分之一判 `stage-thin`、开篇期区间内找不到反转信号判 `early-no-reversal`(条目「前3集必须出现第一个大反转」),集数少于六段时按比例缩放必把某段摊成零集故不产出结论;SK-15 付费卡点位置 `eps.payoffPlacement`——卡点位置就是集尾、兑现位置就是下一集集首,集尾窗口(120 字)内一个卡点信号都没有而本集别处有判 `flat-ending` 并给出全文最后一个信号的位置,集尾有卡点而下一集开篇窗口找不到兑现信号判 `payoff-not-cashed`,末集不判(全剧收束不需要再卡)、全集无信号的段落归剧本段结论不重复报,两组词表都不新写(卡点信号 = SK-07 冲突信号 + SK-14 反转信号,兑现信号 = 打脸四步的反击/释放两步词表)。阶段配比该写什么戏、卡点掐得准不准同属语义面,仍归 LLM 审片(G-10)。消费点是**生产就绪检查**与**问题中心**:浏览器 `Commands.execute('episode.preflight')` 与 CLI `exec episode.preflight` 的 `result.checks` 同一份结论(按主线步序排列:剧本面三条 → 主体面两条 → 分集面两条),剧本面命中、跨镜一致性与分集面结论另在问题中心分别挂低危提醒(`script-craft` / `subject-inconsistent` / 六阶段覆盖按项目挂一条 `eps-structure` / 付费卡点按集挂 `eps-payoff`),**只报不拦**——不进 `blockers`、不改 `ok/status`、不改发布门 G2(只数高/中危)、不新增计费动作与标签),`Skills.covering(stage)`/`forExpert(id)`/`gaps()` 给跨步、专家反查与缺口投影,`Skills.validate(deps)` 由契约测试断言引用键全部命中。四端同一份注册表:index.html(knowledge.js 之后、wf-core.js 之前)+ server.js / cli.js / mcp.js require;`stage` 六步与 `Domain.workflow` 主线步骤键同词表,审片如实标注 `wfStep:false`(尚未进 workflow 步骤集合),条目 `wave` 记落地波次(W2 单源打底 9 / W3 双端贯通 5 / W4 校验闸门 16),校验项每落地一条就把该条目的 `pending` 去掉 `check` 面(契约测试双向对齐:登记的校验项必有实现,实现也必被条目引用)。
...
- **🩺 问题中心**(js/issues.js,第三阶段):`Issues.collect(p)` 把全项目待处理问题聚合为分级清单(高危:失败镜/缺剧本;中:未分镜/分镜过期/素材过期/低分审片/成片过期/主体缺图;低:待确认/跨镜主体参考不一致/剧本方法论提醒/分集方法论提醒),逐项由 `Domain.episodeState` 推导——与流程条/下一步/跑批同一口径不自造状态;跨镜一致性一项由 `Skills.check('subjects',…)`(SK-13 校验项)推导,剧本方法论提醒(开篇钩子锚定/打脸四步/台词单句长度)由 `Skills.check('script',…)`(SK-07/08/09 校验项)推导,分集方法论提醒由 `Skills.check('eps',…)`(SK-14/SK-15 校验项)推导——六阶段结构覆盖判定输入是整张分集表故按项目挂一条、付费卡点位置按集挂;剧本面与分集面都排在未拆镜等早退分支之前(剧本刚写完还没拆镜时正该看得见),本层只做展示文案,低危不影响发布门 G2;命令类问题一键处置(失败镜 → `episode.generateVideos` 带 `shotIds` 子集只重生成失败镜、未分镜 → `episode.generateStoryboard`、成片过期 → `episode.compose`,均 ui 模式保留决策闸),导航类跳对应页面。项目页 tab 行「🩺 问题」按钮(角标=未解决数,`Bus '*'` 订阅实时刷新),弹窗开着时管线事件驱动即时重算。
...
`node tests/cli.smoke.js` — CLI 真实服务端冒烟(二十四轮扩至 61 项断言,零依赖):spawn 临时 server(MOCK_LLM=1)+ 子进程直跑 `cli.js`,覆盖 login/whoami/credits → 项目/分集/剧本 → 主体(含 subject-copy 跨项目复制/同名覆盖)→ 分镜导入/补丁/确认闸 → llm(mock) → usage 成本归集 → jobs/state 往返 → exec 统一领域命令(未注册命令报错含可用清单/preflight blocked 无 error 字段不抛异常/generateStoryboard 服务端工作流 ok 写回分镜)→ **二十四轮 主线前段 headless 起跑(`project-script` 写入整部剧本 → `exec project.splitEpisodes` 无 overwrite 被 blocked 拦下 → `--overwrite --local` 拆集成功 → 正文逐字保留校验 → 首集直接 `exec episode.generateStoryboard` 出分镜表,全程零浏览器)** → state-get 落盘 → workflow 项目级与分集级(不存在项目 exit 4)→ **第四阶段 交付检查(release-check:缺 pid exit 2 用法/不存在 pid exit 4 /基线项目 JSON 结构齐全 7 核心门+overall fail≥2 项+release 无 force exit 5 发布门未通过/release --force 打版本 exit 0 返回 RLS_digest+ver+forced=true + release-check 回填 releases[] 1 条/不存在 pid exit 4)** → 错误路径语义 exit code(3/4/2)→ logout;`HUJING_CONFIG_DIR` 隔离配置目录,测试账号 `__cli__` 用完即清;不触发生图/生视频真实计费调用。
