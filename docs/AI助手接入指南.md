...
- 36 个工具(`hujing_*`),stdio 传输,零依赖;工具调用 = 包装 cli.js,计费/幂等/退费语义与 CLI 完全一致。
...
- 协作记忆播种 `hujing_memory_seed` / 板块迁移 `hujing_memory_migrate`(包装 `cli.js memory seed|migrate`,零 LLM 零计费):种子表与浏览器同一份(`WfCore.memSeed`),`board` 只播指定板块、缺省播全部,幂等(已种过再播 `changed:false`);迁移把旧板名条目原地改到新板名、条目不丢不双写。空板与未知板名一律如实报错,不静默空成功;播种是显式动作——读记忆时不会自动跑。
...
- prompts:`hujing_new_drama`(新剧开工流程)/`hujing_failed_shots`(失败镜排查流程)/`hujing_mid_pipeline`(主线中段主体→分集→分镜→生成的调用顺序、参数出处与断点,按 `segment`/`pid` 现取实况渲染)三个模板,一次拿到正确的工具调用序列。
