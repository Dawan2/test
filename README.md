...
浏览器 SPA(hash 路由,30+ 个 js 模块)
   ...
   │                 系统人设装配口 sbSystem/genPromptSystem = 注册表提示词 + KB 按键取正文
...
| POST | /api/wf/extract-subjects | **提取主体(项目级服务端工作流)**:{pid,mode?('normal'/'fine'),types?,operationId?} → {found:{character,scene,prop},truncated,model};服务端读项目剧本(无 p.script 时取各集正文拼接,超 15000 字截断)→ LLM 语义提取,提示词按生效专家注入方法论段(「主体」板块雇佣 > 全局雇佣)+ 协作记忆按板块召回,提示词/可信性校验/规整 js/wf-core.js 双端同源;计费 llm.extract 服务端定死,失败退费+如实报错;**只出候选不写回 state**——入库口径归调用方(浏览器解析向导按用户勾选合并,CLI `exec project.extractSubjects` headless 全量合并) |
