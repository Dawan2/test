...
async function main() {
  ...
  r = cli('exec', 'subject.generateImage', '--pid', pid, '--args', '{"subjectIds":["sj_not_here"]}'); // 点名的主体不在库里 → 一位也没跑,仍 ok 但回执得说清为什么是 0 位
  report('exec subject.generateImage 一位也没跑 → ok+回执说清原因(note 与前端 digest 同一份)', r.code === 0 && r.out.ok === true && r.out.result.total === 0 && /一位也没跑/.test(r.out.result.note || '') && /1 位不在主体库/.test(r.out.result.note || ''), 'exit=' + r.code + ' ' + JSON.stringify((r.out && r.out.result && r.out.result.note) || r.out));
