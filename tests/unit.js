...
const commandsTests = [
  ...
  { name: 'preflight:result.checks 是剧本+主体+分集+分镜+生成+成片六面并集(按主线步序,摘任一面即红)', fn: async () => {
    const sb = loadCommands();
    ...
    assertEq(checks.map(x => x.skill).join(','),
      ...
      + 'gen.renderCredential,film.subtitleQC,film.deliverContract',
    ...
    /* 与直接跑六面逐字节一致:命令层不得对某一面做二次过滤/降级/改序(沙箱无 Media,ck.online=false) */
...
const skillsTests = [
  ...
  { name: '就绪检查校验面表(源级):面清单双端单源 Skills.preflightStages(),两端只读该表 concat、不各写一份面字面量', fn() {
    ...
    assertEq(stages.reduce((n, st) => n + Skills.check(st, {}).length, 0), 16, '六面共十六条已落地校验项');
