...
- 记账诚实位:`SK-03`/`SK-04`/`SK-23` 的 `infra` 面 `pending` 已按实况清空(三处出口 G-01/G-02/G-03 都已落地),三条 `note` 改为逐处点名各自仍欠的**覆盖余量**(审片侧三步不带人设与记忆召回、记忆补种与旧板块名迁移仍只在浏览器发生、问题中心只报低分不报「未审片」),并由断言钉在三处出口的实况上;缺口标记按**关联索引**口径保留——`gaps()` 只投影 `gaps` 字段、不看 `pending`,故「改 `pending` 会动 `Skills.gaps()` 投影」这条早先的顾虑不成立(有用例把三条退回后逐字节比对钉住)。剩余 `pending` 只有编排面两条(`SK-05`/`SK-26`)与发布门方法论门一条(`SK-29`,待 G-10)。
