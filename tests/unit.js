...
无法判断属于哪类生成意图时【不要返回 plan】,按普通创作顾问对话回答(只给 reply)。`,
  ...
  { name: '漫剧气泡对白人设:经 Prompts.get(comic.bubbleSystem) 取值,缺省逐字节等于收编前的内联字面', fn() {
    ...
    assertEq(census.join(' '), [
      'js/agent-global.js:1', 'js/agent-ops.js:2', 'js/api.js:2', 'js/beatboard.js:1', 'js/episode-util.js:3',
      'js/episodes.js:5', 'js/experts-data.js:16', 'js/experts.js:2', 'js/gsettings.js:2', 'js/persona.js:3',
      'js/plans.js:1', 'js/proj-planner.js:2', 'js/proj-shell.js:1', 'js/proj-upload.js:1', 'js/prompts.js:14',
      'js/role-editor.js:1', 'js/sb-board.js:2', 'js/sb-views.js:1', 'js/wf-core.js:1',
    ].join(' '), '全仓人设字面持有者名单(逐文件计数)');
    assert(!census.some(x => x.startsWith('js/editors.js:')), 'js/editors.js 收编后应已不在持有者名单上');
    assertEq(Prompts.list().filter(x => x.def.startsWith('你是')).length, 14,
      '名单里 js/prompts.js 那 14 处就是注册表 def 本身(注册表条数变了这张名单也要跟着改)');
  } },
