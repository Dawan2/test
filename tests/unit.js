...
const memoryTests = [
  ...
  { name: 'SK-26 记账与实况同步:回流面清 pending 且登记真实步骤,仍欠的自进化面点名 G-11', fn() {
    ...
    assertEq(sk.cmds.join(','),
      'project.extractSubjects,project.splitEpisodes,episode.understanding,episode.generateStoryboard,episode.smartReview',
      '命令面由 steps 推出:有命令出口的五步按主线步序登记,发布留痕两端不在命令注册表、不挂假命令名');
  ...
  { name: '前段四步写入面接线(源级):六处写入点都走 UMD 同一份派生,落点仍是既有 agentMemory 桶', fn() {
    ...
    const srv = files['server.js'];
    assertEq((srv.match(/tree\.agentMemory = WfCore\.memWrite\(tree\.agentMemory,/g) || []).length, 5, '服务端应有五处写入点(审片 + 理解 ×2 + 分镜 + 拆集)');
    ...
    const exSeg = files['cli.js'].slice(files['cli.js'].indexOf("EXEC['project.extractSubjects']"), files['cli.js'].indexOf('CmdRegistry.META.forEach'));
