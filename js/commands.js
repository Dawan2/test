...
(function () {
   ...
   * result.checks 附剧本面/主体面/分集面/成片字幕面校验项结论(Skills.check,纯本地零 LLM 零计费),按主线步序排列——只报不拦:
  ...
  reg('episode.preflight', { risk: 'read', meter: false, label: '生产就绪检查' }, async ({ p, ep }) => {
    ...
    const checks = window.Skills
      ? Skills.check('script', { p, ep }, ck).concat(Skills.check('subjects', { p, ep }, ck), Skills.check('eps', { p, ep }, ck), Skills.check('film', { p, ep }, ck))
      : [];
