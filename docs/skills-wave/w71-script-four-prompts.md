...
六条变异逐一施加、跑 `node tests/unit.js contract` 后复原(复原后 58/58):

| 变异 | 实测行为 | 转红 |
|---|---|---|
| 1 `js/episodes.js` 解说体那步改回内联字面 | 收编退回收编之前 | 2 条(行为面 + 源级) |
| 2 四条键**合成单键**(只留 `narration.system`,四步都取它) | 四步角色定位失真,改一次改掉四条链路 | 2 条(行为面 + 源级) |
| 3 注册表 `def` 改一个字(光影那条句号→叹号) | 缺省不再逐字节相同 | 1 条(行为面) |
| 4 取值口改成 `Prompts.get(key, {})`(不读覆盖表) | 进表了但用户改不到 | 1 条(源级) |
| 5 摘掉 SK-03 的四个键登记 | 新键不进索引、记账对不上账 | 3 条(四类单源键全覆盖 + 源级 + validate) |
| 6 **反向**:把 `js/episode-util.js` 三处策划人设收编 | 仍欠段点名的余量已消失而 `note` 还写着欠 | 1 条(源级) |

变异 2 的具体表现:合成之后 `Prompts.get('reading.system')` 等三条回空串,行为面第 ① 项当场红;源级那条的配对正则也找不到另外三个键。变异 4 只红一条是有意的——覆盖不跟随这件事在行为面看的是注册表取值(那一层仍绿),能拦住它的是源级的写法断言。
