...
{brief}`,
    ...
    /* 制作计划 LLM 规划步(项目页「计划」→ 按用户目标拆步)的人设:角色是"制作计划器"——出的是按序可执行的
     * 制作步骤表,不是对话回复,与上面四份对话人设(单轮/分集面板/全局抽屉/预排)措辞与产物都不同,故独立一条键;
     * 排在对话四条之后:它是同一个助手的另一条产物线,不属于任何一种对话模式。
     * 同样只收人设句:可用领域命令白名单(现取 Commands.list())与返回 JSON 的 title/steps 契约仍由该步就地拼,
     * 不开放覆盖(改坏即整轮拆不出有效步骤,且该步 1 积分失败退费)。 */
    {
