...
- **核心提示词 skill 文件化(js/prompts.js)**:智能分镜系统人设/分镜五角色评审指令与系统人设/本集理解人设/视频提示词改写人设/单镜审片人设/成片审片人设 7 条主线 LLM 提示词集中登记为注册表(`Prompts.get/fill`,评审指令支持 {style}{brief} 变量);其中视频提示词改写人设经 `WfCore.genPromptSystem` 取用,注册表人设句之后按键整条接 KB 抽卡公式+抽卡军规(方法论正文不随覆盖变动);「偏好学习 → 全局默认值 → 核心提示词 skill」在线改写即生效,覆盖存 `settings.promptOverrides`(清空或与默认相同即恢复系统默认,已覆盖条目带标记)。
