...
const TOOLS = [
  ...
  { name: 'hujing_subjects_dedupe', description: '存量重复主体 id 去重(与 hujing_shots_dedupe 同形:默认 dry-run 只报计划;apply=true 才写:首位留原 id、撞车位改发新 id,零上游零计费)', inputSchema: obj({ pid: pidEp.pid, apply: Object.assign({}, B, { description: '真正写库(缺省 dry-run,只报哪些 id 重复、会改成什么)' }) }, ['pid']), build: i => ['subjects-dedupe', i.pid].concat(i.apply ? ['--apply'] : []) },
  ...
  { name: 'hujing_shots_dedupe', description: '存量重复镜头 id 去重(默认 dry-run 只报计划;apply=true 才写:首行留原 id、撞车行改发新 id,零上游零计费)', inputSchema: obj(Object.assign({}, pidEp, { apply: Object.assign({}, B, { description: '真正写库(缺省 dry-run,只报哪些 id 重复、会改成什么)' }) }), ['pid', 'epid']), build: i => ['shots-dedupe', i.pid, i.epid].concat(i.apply ? ['--apply'] : []) },
  ...
  { name: 'hujing_release', description: '发布留痕:过发布门后打版本号并写一条 releases 留痕(digest/ver/checksum/门禁结论,可回滚定位);未过门如实 blocked 不留痕,空项目(无分集)如实拒绝,force 强制打的版本留痕标 forced。零 LLM、零计费', inputSchema: obj({ pid: pidEp.pid, note: Object.assign({}, S, { description: '发布说明(≤500 字)' }), minScore: Object.assign({}, N, { description: '审片均分阈值(缺省取偏好设置,默认 7)' }), force: Object.assign({}, B, { description: '授权位:未过发布门仍强制打版本' }) }, ['pid']), cmd: 'project.release', build: i => ['exec', 'project.release', '--args', JSON.stringify({ pid: i.pid, note: i.note, minScore: i.minScore, force: !!i.force })] },
  ...
  { name: 'hujing_expert_evolve', description: '专家自进化(服务端工作流 /api/wf/evolve-expert):把协作记忆里该专家**生效板块**的沉淀蒸馏为 ≤4 条「进化条款」追加进其 persona,与浏览器专家库的「🧠 进化」按钮同一份判据与同一套提示词。预置专家不可改写,条款落其自定义副本(同一预置只派生一份,副本记 from=派生源;副本被雇佣后条款才进链路)。计费 llm.evolve(1 积分,失败退费);未在任何板块生效、或该板块暂无沉淀,一律在扣费前如实 blocked。**人手动作**:请在用户明确要求时调用,不要挂在任何流程收尾上自动跑——蒸馏不可撤回地改写人设', inputSchema: obj({ expert: Object.assign({}, S, { description: '专家 id 或名称(预置 ex_* / 自定义 cx_*)' }) }, ['expert']), cmd: 'expert.evolve', build: i => ['exec', 'expert.evolve', '--args', JSON.stringify({ expert: i.expert })] },
