...
**`exec <command>`(统一领域命令,与前端 `Commands.execute` 同名同结构)**:`hujing exec episode.preflight --pid X --epid Y`、`episode.generateVideos`(支持 `--confirm-all` 授权全量)、`shot.generateVideo`(`--sid` 支持镜头 id 或序号)、`episode.smartReview`、`episode.compose`、`episode.produce`(一键成片编排);项目级(仅 `--pid`):`subject.generateImage`(缺图主体批量补图,`--args '{"subjectIds":[..]}'` 子集)、`project.extractSubjects`(LLM 剧本提取主体合并入库,计费 llm.extract)。stdout 恒为 `{ ok, status, result?, error?, cost?, next? }`:`cost` 是钱包余额前后差值(含子调用扣费与退费回补),`next` 执行后按 Domain 重推下一步;exit 映射 `ok→0 | blocked→2(no-credits→6 / not-found→4) | failed→5`,blocked 前置拦截(缺剧本/未确认镜/无素材)零调用零计费。**二十一轮:LLM 创作类命令(智能分镜/本集理解/智能审片)已下沉服务端工作流端点 `/api/wf/*`(见 API 表)——CLI/外部 Agent 全链路可跑,计费动作服务端定死,提示词/规整与浏览器 js/wf-core.js 同源。**
...
- **命令元数据双端单源(二十二轮·二批 §2.1)**:`js/cmd-registry.js` UMD 注册 10 条领域命令元数据(name/label/risk/needs/desc/args),commands.js REG 默认值与 `Commands.list()`、cli.js help/EXEC 词表、mcp.js 工具描述全部由注册表生成(此前三处手抄);契约测试断言三端词表集合相等 + cli help 实跑含全部命令。
