...
浏览器 SPA(hash 路由,30+ 个 js 模块)
   ...
   │                 校验型扩展点 CHECKS 已落地七条:剧本面 SK-07/08/09(开篇钩子锚定 / 打脸四步
   │                 完备性 / 台词单句长度),主体面 SK-12 分镜引用主体完备性(单镜引用能否落到
   │                 主体、有无可喂模型的真实参考图)与 SK-13 跨镜头主体一致性(同一主体跨镜锁到
   │                 的参考图/引用名一不一致),分镜面 SK-18 景别衔接(级差经 WfCore.sizeGap 判
   │                 连续同景别成串/两极对切缺过渡/整集无隔级递进),成片面 SK-28 字幕时间轴与
   │                 阅读速度(按合成时间轴段判烧录截断/读速/停留/空字幕轨),判定现取 Domain 与
   │                 WfCore,经就绪检查回执 result.checks 与问题中心消费,只报不拦、零 LLM 零计费
   ...
   │                 参考不一致/景别衔接/成片字幕读不顺 全项目聚合(Domain 与 skill 校验项单源),
