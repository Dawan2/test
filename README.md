...
| POST | /api/wf/agent | **Agent 单轮对话(服务端管线)**:{pid,epid?,text,scope?,operationId?} → {reply,thinking,ops,receipts};服务端拼装对话注入(KB.block/雇佣专家 personaNote/协作记忆 memBlock 按 scope 召回/工作台状态摘要/分镜表压缩,提示词 js/wf-core.js 双端函数),计费 llm.agent 服务端定死(失败退费);ops 只解析不执行——run 类领域命令经注册表白名单过滤+参数整形(≤5 条),由调用方按需走 `hujing exec`/`Commands.execute` 执行并各自计费;浏览器工作台面板仍走 agent.js 原路径(数据类 ops/预览确认/冲突闸是工作台语义) |
